...
class EditUsersWindow(MDScreen):
    ...
    def update_membersip(self):
        ...
        UrlRequest(url, req_body=updated_section_json, method='PUT', on_success=self.users_updated)

    def users_updated(self, req, result):
        section_name = result['section_name']
        self.parent.parent.switch_screen("Sections Screen", "right")
        Snackbar(text = f'{section_name} Updated').open()

    def delete_users_dialog(self):
    ...
    def delete_users(self, *args):
        ...
        UrlRequest(url, method='DELETE', on_success=self.users_deleted)
    
    def users_deleted(self, request, result):
        self.close_dialog()
        self.parent.parent.switch_screen("Sections Screen", "right")
...
class AddUsersWindow(MDScreen):
    ...
    def create_memberstatus(self):
        ...
        UrlRequest(self.users_url, req_body=new_section_json, on_success=self.users_created)

    def users_created(self, req, result):
...
class UsersHomeWindow(MDScreen):
    ...
    def __init__(self, **kw):
        super().__init__(**kw)
        UrlRequest(self.users_url, on_success=self.get_maritalstatus)
    
    def get_maritalstatus(self, request, result):
        ...
        self.set_search_maritalstatus()

    def set_search_maritalstatus(self, text="", search=False):
        '''function that creates a seachable list and 
            append list items to the sections main screen'''

        def add_maritalstatus_item(name):
        ...
        for item in self.data:
            ...
            if search:
                if text in name:
                    add_maritalstatus_item(name)
            else:
                add_maritalstatus_item(name)

    def edit_maritalstatus(self, name):
        '''function for searching a particular section in database'''
        id = name[0]
        url = f'{self.users_url}{id}'
        UrlRequest(url, on_success=self.edit_section_screen)

    def edit_maritalstatus_screen(self, request, result):
...
class UsersWindow(MDScreen):
    users_screen_manager = ObjectProperty()

    def switch_screen(self, screen_name, transition):
        self.users_screen_manager.transition.direction = transition
        self.users_screen_manager.current = screen_name
