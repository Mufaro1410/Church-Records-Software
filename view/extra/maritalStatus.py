...
Builder.load_file(os.path.join(dirname, "maritalStatus.kv"))

class EditMaritalStatusWindow(MDScreen):
    memberstatus_url = 'http://127.0.0.1:8000/marital/'
...
class MaritalStatusHomeWindow(MDScreen):
    ...
    def __init__(self, **kw):
        super().__init__(**kw)
        UrlRequest(self.maritalstatus_url, on_success=self.get_maritalstatus)
...
class MaritalStatusWindow(MDScreen):
    marital_status_screen_manager = ObjectProperty()

    def switch_screen(self, screen_name, transition):
        self.marital_status_screen_manager.transition.direction = transition
        self.marital_status_screen_manager.current = screen_name

class MaritalStatusApp(MDApp):
    def build(self):
        return MaritalStatusWindow()

if __name__ == "__main__":
    MaritalStatusApp().run()