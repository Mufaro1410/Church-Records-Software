...
router = APIRouter(prefix='/membership', tags=['Membership Status'])

# create Membership Status
@router.post('/', response_model=schemas.MembershipStatusRead)
def create_membership_status(membership: schemas.MembershipStatusCreate, db: Session = Depends(get_db)):
    new_membership = models.membershipStatus(**membership.dict())
